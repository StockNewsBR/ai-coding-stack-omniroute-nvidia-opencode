...
test("A2A desabilitado → 503 (mesmo gate do JSON-RPC)", serial, async () => {
  const res = await tasksRoute.POST(delegationRequest(VALID_BODY));
  assert.equal(res.status, 503);
});

test(
  "com OMNIROUTE_API_KEY configurada, bearer errado → 401 e bearer certo passa",
  serial,
  async () => {
    await enableA2A();
    await settings.updateSettings({ requireLogin: true, password: "synthetic-test-password" });
...
test(
  "delegação válida → 201 com o task_id do hub; requirements derivados da skill do card",
  serial,
  async () => {
    await enableA2A();
    ...
    assert.equal(
      sent.requirements.cli,
      "claude",
      "skill conductor-cli-claude vira requirements.cli"
    );
...
test(
  "sem repo na metadata → 400 (delegação exige repo); skill não-conductor → 400",
  serial,
  async () => {
    await enableA2A();
