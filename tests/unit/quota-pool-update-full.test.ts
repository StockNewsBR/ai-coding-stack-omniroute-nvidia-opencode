...
test("PoolUpdateSchema accepts groupId and connectionIds together with name", serial, () => {
...
test("PoolUpdateSchema.data.groupId equals the parsed value", serial, () => {
  const result = PoolUpdateSchema.safeParse({ groupId: "my-group" });
  assert.ok(result.success);
  assert.equal(result.data?.groupId, "my-group");
});

test("PoolUpdateSchema.data.connectionIds deep-equals the input array", serial, () => {
...
test("PoolUpdateSchema rejects connectionIds with empty string element", serial, () => {
...
test("PoolUpdateSchema rejects connectionIds as empty array", serial, () => {
  const result = PoolUpdateSchema.safeParse({ connectionIds: [] });
  assert.equal(result.success, false, "Empty connectionIds array should be rejected");
});

test("PoolUpdateSchema accepts empty object (no-op still works)", serial, () => {
  assert.ok(PoolUpdateSchema.safeParse({}).success);
});

test("PoolUpdateSchema accepts only groupId (partial update)", serial, () => {
...
test(
  "updatePool with new connectionIds triggers combo re-sync (openrouter → baidu)",
  serial,
  async () => {
    ...
    assert.deepEqual(
      reread.connectionIds,
      [idB],
      "pool.connectionIds must contain only baidu conn"
    );
  }
);

test(
  "PATCH route sequence (remove→update→sync) prunes OLD-provider combos on switch",
  serial,
  async () => {
    ...
    assert.equal(
      orphans.length,
      0,
      `OLD openrouter combos must be pruned; found ${orphans.length}`
    );
    assert.ok(
      after.some((c) => {
        const p = parseQuotaModelName(c.name);
...
test("updatePool with groupId persists the new group assignment", serial, () => {
...
test("updatePool without connectionIds leaves connection membership untouched", serial, () => {
  const pool = poolsDb.createPool({
...
test(
  "PoolUpdateSchema path: groupId flows through to updatePool (schema→db round-trip)",
  serial,
  () => {
...
test(
  "PoolUpdateSchema path: connectionIds flows through to updatePool (schema→db round-trip)",
  serial,
  () => {
    const pool = poolsDb.createPool({
