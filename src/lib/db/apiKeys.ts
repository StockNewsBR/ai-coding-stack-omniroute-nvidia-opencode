...
/** Resolve an exact id or one unambiguous non-secret id prefix. */
export async function getApiKeyByIdOrSafePrefix(identifier: string) {
  const normalized = identifier.trim();
  if (!normalized) return null;
  const exact = await getApiKeyById(normalized);
  if (exact) return exact;
  if (!/^[A-Za-z0-9-]{8,36}$/.test(normalized)) return null;

  const db = getDbInstance() as ApiKeysDbLike;
  const rows = db
    .prepare<ApiKeyRow>("SELECT id FROM api_keys WHERE id LIKE ? LIMIT 2")
    .all(`${normalized}%`);
  if (rows.length !== 1 || typeof rows[0]?.id !== "string") return null;
  return getApiKeyById(rows[0].id);
}
